...
def main(args):
    ...
    device = "cpu" if args.cpu else f"cuda:{args.device}"
    ...
    if args.load_model == "":
        ...
        if isinstance(csd, dict) and "net" in csd:
    ...
    msfa_kernel = torch.zeros(
        MSFA.shape[0] * MSFA.shape[1], 1, MSFA.shape[0] * 2, MSFA.shape[1] * 2
    ).to(device)
    ...
    for cnt, (idx, mosaic, pan) in enumerate(tqdm.tqdm(tzip(ids, mosaics, pans))):
        ...
        with torch.no_grad():
            est_f, est_g = fuse_net(mosaic_tensor, pan_tensor, msfa_kernel, inference_flag=True)
            hrms_tensor = (est_f + est_g).detach()[0].cpu()
