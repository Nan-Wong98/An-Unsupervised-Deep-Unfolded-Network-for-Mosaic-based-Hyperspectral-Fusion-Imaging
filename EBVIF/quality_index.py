...
def calc_qi(img1, img2, patch_size=16, average=True):  
    ...
    mean1 = img1.mean(dim=(3, 5), keepdim=True)                                                  
    mean2 = img2.mean(dim=(3, 5), keepdim=True)  
    var1 = torch.var(img1, dim=(3, 5))                                                      
    var2 = torch.var(img2, dim=(3, 5))  
    covariance = ((img1 - mean1) * (img2 - mean2)).mean(dim=(3, 5))                                                        
...
def calc_qnr_mosaic(hrms, mosaic, pan, msfa_kernel, patch_size=32, scale_factor=2, p=1, q=0.5, alpha=1, beta=3):
