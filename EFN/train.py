...
def train(fuse_net: nn.Module, degrade_r: nn.Module, optimizers, train_dataloader, val_dataloader, args):
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        if psnr_avg > best_psnr:
            ...
            best_epoch = epoch
            torch.save({"fuse_net": fuse_net.state_dict(), "degrade_r": degrade_r.state_dict()}, os.path.join(args.dir_model, "best_{}.pth".format(epoch)))
        
        # save model at some frequency
        if epoch % args.save_freq == 0:
            torch.save({"fuse_net": fuse_net.state_dict(), "degrade_r": degrade_r.state_dict()}, os.path.join(args.dir_model, f"{epoch}.pth"))
...
def main(args):
    ...
    if args.resume != "":
        ...
        degrade_r.load_state_dict(torch.load(backup_pth)[f"degrade_r"], strict=False)
    else:
