...
from GetDataSet import MakeDatasetforPansharpening
        ...
        with torch.no_grad():
            for cnt, data in enumerate(val_dataloader):
                try:
                    ...
                    mosaic, lrms, pan, hrms = data[0].cpu(), data[1].cpu(), data[2].cpu(), data[3].cpu()
                    ps_net = ps_net.cpu()
                    fused = ps_net(lrms, pan).detach()
...
def main(args):
    ...
    train_set = MakeDatasetforPansharpening(args, "train", demosaic_net)
    train_dataloader = DataLoader(dataset=train_set, batch_size=args.batch_size, shuffle=True,
                                  num_workers=args.num_workers, pin_memory=True)
    val_set = MakeDatasetforPansharpening(args, "test", demosaic_net)
