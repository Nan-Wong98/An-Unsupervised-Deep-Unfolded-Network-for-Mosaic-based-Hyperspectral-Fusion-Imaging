from model import Network, Proj_Net, Degrade_R, degrade_dm
import torch
import os
import numpy
...
def main(args):
    ...
    if args.load_model == "":
        ...
        for i in range(args.iters+1):
            proj_nets[i].load_state_dict(csd[f"proj_net{i}"], strict=False)
            degrade_rs[i].load_state_dict(csd[f"degrade_r{i}"], strict=False)
        
    fuse_net = fuse_net.to(device)
    fuse_net.eval()
    for i in range(args.iters+1):
        proj_nets[i] = proj_nets[i].to(device)
        proj_nets[i].eval()
        degrade_rs[i] = degrade_rs[i].to(device)
        degrade_rs[i].eval()
        ...
        with torch.no_grad():
            try:
                ...
                for t in range(args.iters):
                    if args.shared:
                        pan_from_hrms = degrade_rs[-1](hrms_tensor)
                    else:
                        pan_from_hrms = degrade_rs[t](hrms_tensor)
                    mosaic_from_hrms = degrade_dm(hrms_tensor, msfa_kernel)
                    delta_mosaic = mosaic_tensor - mosaic_from_hrms
                    delta_pan = pan_tensor - pan_from_hrms
                    if args.shared:
                        delta_hrhsi = proj_nets[-1](delta_mosaic, delta_pan)
                    else:
                        delta_hrhsi = proj_nets[t](delta_mosaic, delta_pan)
                    hrms_tensor = hrms_tensor + delta_hrhsi
            except:
                ...
                for i in range(args.iters+1):
                    proj_nets[i] = proj_nets[i].cpu()
                    degrade_rs[i] = degrade_rs[i].cpu()
                hrms_tensor = fuse_net(mosaic_tensor, pan_tensor).detach()
                for t in range(args.iters):
                    if args.shared:
                        pan_from_hrms = degrade_rs[-1](hrms_tensor)
